...
export const edges = [
  ...
  {
    type: EdgeType.parallelogram_alt,
    open: "[\\",
    close: "\\]",
  } as IEdgeRender,
...
export function parseCode(
  ...
  styles: Array<string> = [],
  globalNodeClickName = "window.mermaidClick"
): string {
  if (Array.isArray(nodes) && nodes.length > 0) {
    const parseCode = GraphStype(type) + "\n";
    const groupNodes = getGroupNodes(nodes, globalNodeClickName);
    const code =
      parseCode + groupNodes + customStyle(styles, nodes).join(" \n");
    return code;
  } else {
    return "";
  }
...
function getGroupNodes(
  ...
  for (const groupName of innerMap.keys()) {
    ...
    if (groupName !== "unGroup") {
      result.push(buildGroup(groupName, narr, nids, globalNodeClickName));
    } else {
      const nodesStr = buildNodesStr(narr, globalNodeClickName);
...
function buildGroup(
  ...
  const innerNodesStr = buildNodesStr(innerNodes, globalNodeClickName);
  const outNodeStr = buildNodesStr(outNodes, globalNodeClickName);
  return `subgraph ${groupName} \n ${innerNodesStr} end \n ${outNodeStr}`;
}

function buildNodesStr(
  nodes: Array<MermaidNode>,
  globalNodeClickName: string
): string {
  ...
  return (
          ...
          return item.next
            .map((n, index) => {
              ...
              if (next != null && typeof next != "undefined") {
                return `${render(item)}${buildLink(item, index)}${render(
                  next
                )}`;
              } else {
                //TODO error
    ...
    nodes
      ...
      .map((item) => {
        return `click ${item.id} ${globalNodeClickName}`;
      })
      .join("\n") +
    "\n" +
    nodes
